...
<body>
	...
	<div class="main-title">
		<h2>
			<?php echo ($list['id']?'编辑':'新增'); ?>购物中心
		</h2>
	</div>
	<form action="<?php echo U();?>" method="post" class="form-horizontal">
		<div class="form-item">
			<label class="item-label">购物中心名称：</label>
			<div class="controls">
				<input type="text" class="text input-large" name="name" value="<?php echo ((isset($list["name"]) && ($list["name"] !== ""))?($list["name"]):''); ?>">
			</div>
		</div>
        <div class="form-item">
            <label class="item-label">选择城市</label>
            <select name="city_id" id="city">
                <option value="" >请选择</option>
                <?php if(is_array($city)): foreach($city as $key=>$v): ?><option value="<?php echo ($v['id']); ?>" <?php if(($list["city_id"]) == $v[id]): ?>selected<?php endif; ?> ><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
            </select>
        </div>
        <div class="form-item">
            <label class="item-label">地址：</label>
            <div class="controls">
                <input type="text" class="text input-large" name="address" value="<?php echo ((isset($list["address"]) && ($list["address"] !== ""))?($list["address"]):''); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">经度：</label>
            <div class="controls">
                <input type="text" class="text input-large" name="longitude" value="<?php echo ((isset($list["longitude"]) && ($list["longitude"] !== ""))?($list["longitude"]):''); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">纬度：</label>
            <div class="controls">
                <input type="text" class="text input-large" name="latitude" value="<?php echo ((isset($list["latitude"]) && ($list["latitude"] !== ""))?($list["latitude"]):''); ?>">
            </div>
        </div>
		<div class="form-item">
            <label class="item-label">标志：</label>

            <div class="controls">
                <input type="file" id="uploadPic">
                <input type="hidden" name="image" id="image" value="<?php echo ((isset($list["image"]) && ($list["image"] !== ""))?($list["image"]):''); ?>"/>
                <div class="upload-img-box">
                    <?php if(!empty($list["image"])): ?><div class="upload-pre-item"><img src="/onethink<?php echo ($list["image"]); ?>"/></div><?php endif; ?>
                </div>
                <script type="text/javascript">
                    ...
                    //     "buttonText"      : "上传图片",
                    //     "uploader"        : "<?php echo U('Mall/image_upload');?>",
                    //     "width"           : 120,
                    //     'removeTimeout'	  : 1,
                    ...
                    //     console.log(data);
                    //     var dataNew = JSON.parse($.trim(data));
                    //     var src = '';
                    //     if(dataNew.status){
                    //         src = dataNew.url || '/onethink' + dataNew.data.image;
                    //         console.log(src);
                    //         $('#image').val(dataNew.data.image);
                    //         $("#image").parent().find('.upload-img-box').html(
                    //                         '<div class="upload-pre-item"><img src="' + src + '"/></div>'
                    //         );
                    //     } else {
                    //         var that;
                    //         updateAlert(dataNew.info);
                    //         setTimeout(function(){
