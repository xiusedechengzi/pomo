...
<body>
	...
	<div class="cf">
		...
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="keys" class="search-input" value="<?php echo I('keys');?>" placeholder="请输入购物中心名称或者ID">
		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo (get_city($list["city_id"])); ?></td>
						<td><img src="/onethink<?php echo ($list["image"]); ?>" width="40px" height="40px"/></td>
						<td><?php echo ($list["longitude"]); ?></td>
						<td><?php echo ($list["latitude"]); ?></td>
						<td><?php echo (change_time($list["addtime"])); ?></td>
						<td>
							<a title="编辑" href="<?php echo U('edit?id='.$list['id']);?>" class="btn">编辑</a>
							<a class="btn" title="删除" href="<?php echo U('del?id='.$list['id']);?>">删除</a>
							<a class="btn" title="组合包" href="<?php echo U('package?id='.$list['id']);?>">组合包</a>
						</td>
