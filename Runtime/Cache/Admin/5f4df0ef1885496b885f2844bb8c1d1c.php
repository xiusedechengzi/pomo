...
<body>
	...
	<div class="cf">
		...
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="keys" class="search-input" value="<?php echo I('keys');?>" placeholder="请输入商场名称或者ID">
		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo ($list["mall_id"]); ?></a></td>
						<td><?php echo ($list["url"]); ?></a></td>
						<td><?php echo ($list["key"]); ?></a></td>
						<td><?php echo ($list["update_id"]); ?></td>
