...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('User/addf');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('User/changeFuserStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('User/changeFuserStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('User/changeFuserStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('User/changeFuserStatus?method=forbidUser&id='.$vo['id']);?>" class="btn ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('User/changeFuserStatus?method=resumeUser&id='.$vo['id']);?>" class="btn ajax-get">启用</a><?php endif; ?>
				<a title="编辑" href="<?php echo U('addf?id='.$vo['id']);?>" class="btn">编辑</a>
				<a href="<?php echo U('AuthManager/group?id='.$vo['uid']);?>" class="btn confirm ajax-get">删除</a>
                </td>
